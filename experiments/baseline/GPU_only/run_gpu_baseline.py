"""
GPU-only Baseline Test for GraphSAGE on Flickr Dataset

This script runs the complete GraphSAGE model on GPU without any partitioning,
...
def run_gpu_baseline(num_runs=10, warmup_runs=2):
    """
    Run GraphSAGE inference on GPU
    ...
    # Move output back to CPU for statistics
    output = output.cpu()
...
def save_results(results, output_file='gpu_baseline_results.txt'):
    """Save results to file"""
    output_path = Path(__file__).parent / output_file

    with open(output_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("GPU-only Baseline Results\n")
        f.write("="*80 + "\n\n")
        ...
        f.write(f"  GPU Memory: {results['gpu_memory_mb']:.2f} MB\n\n")
...
if __name__ == "__main__":
    # Run baseline test
    results = run_gpu_baseline(num_runs=10, warmup_runs=2)
    ...
    if results is not None:
        ...
        print("\n" + "="*80)
        print("GPU Baseline Test Completed!")
        print("="*80)
    else:
        print("\n" + "="*80)
        print("GPU Baseline Test Failed - CUDA not available")
        print("="*80)
