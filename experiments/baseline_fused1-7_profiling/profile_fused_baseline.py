#!/usr/bin/env python3
"""
Fused 1-7 Baseline Profiling Script

Tests FusedBlock0_7 (all 7 stages combined) on CPU/GPU as a fair baseline
for comparing against the multi-device pipeline.

This baseline uses the same stage implementations as the pipeline,
but runs entirely on a single device.

Usage:
    python profile_fused_baseline.py --export          # Export models
    python profile_fused_baseline.py --measure-cpu     # Measure CPU only
    python profile_fused_baseline.py --measure-gpu     # Measure GPU only
    python profile_fused_baseline.py --measure-cpugpu  # Measure CPU and GPU
    python profile_fused_baseline.py --all             # Full workflow
"""
...
def export_models():
    ...
    print("=" * 70)
    print("Exporting FusedBlock0_7 Models (Stage 1-7 Combined)")
    print("=" * 70)
    ...
    # Export ONNX
    onnx_path = MODELS_DIR / "fused_block0_7.onnx"
    ...
    for device in ['CPU', 'GPU']:
        ir_path = MODELS_DIR / f"fused_block0_7_{device.lower()}.xml"
...
def measure_latency_openvino(ir_path, device, dummy_input, num_warmup=10, num_iterations=50):
    """Measure latency using OpenVINO async API for accurate timing"""
    try:
...
def generate_summary(results):
    ...
    summary_lines = [
        "# Fused Block 0-7 Baseline Results",
        "",
