#!/usr/bin/env python3
"""
GAT Fused Baseline Profiling Script

Tests FusedGAT (all 7 stages combined) on CPU/GPU as a fair baseline
for comparing against the multi-device pipeline.

Usage:
    python profile_gat_baseline.py --export          # Export models
    python profile_gat_baseline.py --measure-cpu     # Measure CPU only
    python profile_gat_baseline.py --measure-gpu     # Measure GPU only
    python profile_gat_baseline.py --measure-cpugpu  # Measure CPU and GPU
    python profile_gat_baseline.py --all             # Full workflow
"""
...
def export_models():
    ...
    print("=" * 70)
    print("Exporting FusedGAT Models (7 Stages Combined)")
    print("=" * 70)
    ...
    # Create model
    model = FusedGAT(FEATURE_DIM, OUT_DIM)
    model.eval()

    # Export ONNX
    onnx_path = MODELS_DIR / "fused_gat.onnx"
    ...
    for device in ['CPU', 'GPU']:
        ir_path = MODELS_DIR / f"fused_gat_{device.lower()}.xml"
...
def measure_cpugpu(test_cases, config, devices=['CPU', 'GPU']):
    ...
    print("\n" + "=" * 70)
    print(f"Measuring {'/'.join(devices)} Latencies (FusedGAT)")
    print("=" * 70)
    ...
    for device in devices:
        ...
        for case in test_cases:
            ...
            key = f"fused_gat,{nodes},{edges},{device}"
            results[key] = result
...
def generate_summary(results):
    ...
    summary_lines = [
        "# Fused GAT Baseline Results",
        "",
        "Single-device baseline using all 7 stages fused together.",
        "",
        "## Results Summary",
        "",
    ...
    summary_path = RESULTS_DIR / "gat_summary.md"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GAT Fused Baseline Profiling")
    ...
    if args.platform:
        ...
        RESULTS_DIR = SCRIPT_DIR / 'results' / PLATFORM / 'gat'
    ...
    if args.measure_cpu:
        ...
        save_results(results, "fused_gat_cpu.json")

    if args.measure_gpu:
        results = measure_gpu(test_cases, config)
        all_results.update(results)
        save_results(results, "fused_gat_gpu.json")

    if args.measure_cpugpu:
        results = measure_cpugpu(test_cases, config)
        all_results.update(results)
        save_results(results, "fused_gat_cpugpu.json")

    # Generate summary
    if args.analyze:
        if not all_results:
            cpugpu_file = RESULTS_DIR / "fused_gat_cpugpu.json"
