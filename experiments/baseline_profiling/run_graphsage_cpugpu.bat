@echo off
REM ========================================================================
REM GraphSAGE - CPU + GPU Testing
REM ========================================================================
setlocal EnableDelayedExpansion

echo ========================================================================
echo Baseline Profiling - GraphSAGE (CPU + GPU)
echo ========================================================================
echo.
...
echo [Step 1/2] Exporting GraphSAGE models...
python -c "
import sys
sys.path.insert(0, '.')
...
model = GraphSAGE1Layer(FEATURE_DIM, OUT_DIM)
model.eval()

# Export ONNX
onnx_path = MODELS_DIR / 'graphsage_dynamic.onnx'
with torch.no_grad():
...
for device in ['CPU', 'GPU']:
    ir_path = MODELS_DIR / f'graphsage_{device.lower()}.xml'
    convert_to_ir(onnx_path, ir_path)
    print(f'Exported: {ir_path.name}')
"

echo.
echo [Step 2/2] Measuring GraphSAGE latencies...
python -c "
import json
import sys
sys.path.insert(0, '.')
...
for device in ['CPU', 'GPU']:
    ...
    for case in test_cases:
        ...
        key = f'graphsage,{nodes},{edges},{device}'
        results[key] = result
        if result['failed']:
            print(f'FAILED')
        else:
...
with open(RESULTS_DIR / 'graphsage_cpugpu.json', 'w') as f:
    json.dump(results, f, indent=2)
print(f'\nResults saved to: results/graphsage_cpugpu.json')
"

echo.
echo GraphSAGE CPU+GPU testing complete!
pause
