@echo off
REM ========================================================================
REM GCN - CPU + GPU Testing
REM ========================================================================
setlocal EnableDelayedExpansion

echo ========================================================================
echo Baseline Profiling - GCN (CPU + GPU)
echo ========================================================================
echo.
...
echo [Step 1/2] Exporting GCN models...
python -c "
import sys
sys.path.insert(0, '.')
...
model = GCN1Layer(FEATURE_DIM, OUT_DIM)
model.eval()

# Export ONNX
onnx_path = MODELS_DIR / 'gcn_dynamic.onnx'
with torch.no_grad():
...
for device in ['CPU', 'GPU']:
    ir_path = MODELS_DIR / f'gcn_{device.lower()}.xml'
    convert_to_ir(onnx_path, ir_path)
    print(f'Exported: {ir_path.name}')
"

echo.
echo [Step 2/2] Measuring GCN latencies...
python -c "
import json
import sys
sys.path.insert(0, '.')
...
with open(RESULTS_DIR / 'gcn_cpugpu.json', 'w') as f:
    json.dump(results, f, indent=2)
print(f'\nResults saved to: results/gcn_cpugpu.json')
"

echo.
echo GCN CPU+GPU testing complete!
pause
