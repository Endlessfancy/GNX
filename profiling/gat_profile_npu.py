#!/usr/bin/env python3
"""
GAT NPU Single Test Script - Tests one (nodes, stage) combination with all edges

GAT 7-Stage Decomposition:
    Stage 1: LINEAR           - NPU supported
    Stage 2: GATHER_BOTH      - NPU supported
    Stage 3: ATTENTION_SCORE  - NPU supported
    Stage 4: ATTENTION_SOFTMAX - NPU SKIP (scatter)
    Stage 5: MESSAGE_WEIGHTED - NPU supported
    Stage 6: REDUCE_SUM       - NPU SKIP (scatter)
    Stage 7: ACTIVATE         - NPU supported

Usage:
    python gat_profile_npu.py --nodes 1000 --stage 1
    python gat_profile_npu.py --nodes 1000 --stage 1 --edges 2000,5000,10000
...
NPU_SUPPORTED_STAGES = [1, 2, 3, 5, 7]
...
def generate_dummy_input(stage_id, num_nodes, num_edges, feature_dim=500):
    """Generate dummy input for a GAT stage"""
    torch.manual_seed(42)

    if stage_id == 1:
        # Stage 1: LINEAR - x[N, F]
        x = torch.randn(num_nodes, feature_dim)
        return (x,)
    elif stage_id == 2:
        # Stage 2: GATHER_BOTH - Wx[N, F'], edge_index[2, E]
        Wx = torch.randn(num_nodes, feature_dim)
        edge_index = torch.randint(0, num_nodes, (2, num_edges))
        return (Wx, edge_index)
    elif stage_id == 3:
        # Stage 3: ATTENTION_SCORE - Wx_i[E, F'], Wx_j[E, F']
        Wx_i = torch.randn(num_edges, feature_dim)
        Wx_j = torch.randn(num_edges, feature_dim)
        return (Wx_i, Wx_j)
    elif stage_id == 5:
        # Stage 5: MESSAGE_WEIGHTED - Wx_j[E, F'], alpha[E]
        Wx_j = torch.randn(num_edges, feature_dim)
        alpha = torch.rand(num_edges)
        return (Wx_j, alpha)
    elif stage_id == 7:
        # Stage 7: ACTIVATE - h[N, F']
...
def main():
    parser = argparse.ArgumentParser(description='GAT NPU Single Test')
    parser.add_argument('--nodes', type=int, required=True, help='Number of nodes')
    parser.add_argument('--stage', type=int, required=True, choices=NPU_SUPPORTED_STAGES,
                       help='Stage ID (4/6 skipped for NPU - scatter operations)')
    ...
    print(f"=" * 60)
    print(f"GAT NPU Test: Stage {args.stage}, Nodes={args.nodes}")
    print(f"NPU Skip Stages: 4, 6 (scatter operations)")
