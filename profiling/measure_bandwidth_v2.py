"""
分离测量 OpenVINO 输入/输出带宽 V2 (直接测量)

改进点:
- 使用 Identity 模型，分离计时
- 输入带宽: 从 start_async + wait 测量 (扣除极小的 Identity 计算)
...
def measure_dynamic_device(device, core, sizes=TEST_SIZES):
    ...
    """
    print(f"\n{'='*70}")
    print(f"测试设备: {device} (动态形状, V2 分离测量)")
    print(f"{'='*70}")
...
def measure_static_device(device, core, sizes=NPU_TEST_SIZES):
    ...
    """
    print(f"\n{'='*70}")
    print(f"测试设备: {device} (静态形状, V2 分离测量)")
    print(f"{'='*70}")
...
def main():
    print("=" * 70)
    print("OpenVINO 分离带宽测量 V2 (直接测量输入/输出)")
    print("=" * 70)
    print()
    print("方法 V2:")
    ...
    print("  V2: Identity 分离计时 → 直接测输入/输出")
    print()
    ...
    output_file = output_dir / 'bandwidth_v2.json'
